...
# A module describing a path created by a users that may have dynamic points, meaning points that update according to the user's input.
#
# Author::    Andy Likuski (andy@likuski.org)
# License::   Distributes under the same terms as Ruby
class Simple_Dynamic_Path
  include Dynamic_Path

  def initialize(path_to_point_data)
    @path_to_point_data = path_to_point_data
  end

  # A collection of Dynamic_Pair instances in the order selected by the user
  def dynamic_pairs
    @path_to_point_data.data_pairs
  end
